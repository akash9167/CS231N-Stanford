...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    score1 = np.maximum(X.dot(W1)+b1, 0)
    ...
    score1 = X1
    score2 = X2
    dW2 = (score1.T).dot(scores - Y)/N + reg*W2
    dB2 = (dW2.T).dot(np.ones(np.shape(score1[0])))/N
    err = scores -  Y
    ...
    dW1 = X.T.dot(a)/N + reg*W1
    dB1 = (dW1.T).dot(np.ones(np.shape(X[0])))/N
